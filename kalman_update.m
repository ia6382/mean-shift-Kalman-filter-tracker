...
% x(:) - E[X | y(:, 1:t-1)] prior mean //ni prior ampak previous posterior
...
%debug plot
hold on;
plot(xpred(1), xpred(2), '-b*');
drawElipse(xpred(1), xpred(2), Vpred(1,1), Vpred(2,2), '-b');
    
plot(y(1), y(2), '-r*');
drawElipse(y(1), y(2), R(1,1), R(2,2), '-r');

plot(xnew(1), xnew(2), '-g*');
drawElipse(xnew(1), xnew(2), Vnew(1,1), Vnew(2,2), '-g');
hold off;
